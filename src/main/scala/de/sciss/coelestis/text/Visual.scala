...
package de.sciss.coelestis.text
...
//    is.close()
//    res
    new Font("Droid Sans Armenian", Font.PLAIN, 1)
  }
      ...
      Processor[Unit]("saveFrameSeriesAsPNG") { p =>
        ...
        while (frame < numFrames) {
          ...
          def mix(a: Situation, b: Situation, w2: Double): Situation = {
            ...
            val width           = (a.config.width     * w1 + b.config.width      * w2 + 0.5).toInt
            val height          = (a.config.height    * w1 + b.config.height     * w2 + 0.5).toInt
            ...
            val config          = Config(width = width, height = height, lineWidth = lineWidth, speedLimit = speedLimit,
          ...
          execOnEDT {
            ...
            text        = thisSit.text
            imageSize   = new Dimension(thisSit.config.width, thisSit.config.height)
          ...
          if (frameSave >= 0) {
            val f = mkF()
            execOnEDT {
              saveFrameAsPNG(f, width = thisSit.config.width, height = thisSit.config.height)
              // _dsp.damageReport() // force complete redrawing
        ...
        @tailrec def loopPlop(sq: Vec[VisualVertex]): Unit = sq match {
          case head +: tail =>
            ...
            execOnEDT {
              ...
              saveFrameAsPNG(f, width = lastSit.config.width, height = lastSit.config.height)
              animationStep()
            }
            frame += 1
            loopPlop(tail)
